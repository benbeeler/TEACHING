...
%Boundary conditions
bbottom = applyBoundaryCondition(model,'Edge',1,'u',@T_BC);
bouter = applyBoundaryCondition(model,'Edge',2,'u',@T_BC);
bmid = applyBoundaryCondition(model,'Edge',3,'u',@T_BC);
...
function Tbnd = T_BC(region, state) 
global LHR0 lngth Rf;
gamma = 1.3;
mdot = 0.25; 
CPW = 4200;
