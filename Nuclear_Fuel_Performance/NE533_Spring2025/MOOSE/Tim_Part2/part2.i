...
coolant_avg_vel = 1.264 # 4.81 # m/s 15.8 f/s
water_density = 769 # kg/m3
water_heat_cap = 3080 # J/kg-K
water_heat_trans_coeff = 30000 # W/m2-K
deltaT = '${fparse volumetric_heat_production_rate*pi*fuel_radius^2*core_height / (coolant_avg_vel * (rod_pitch^2 - pi * (rod_dia/2)^2) * water_density * water_heat_cap)}'
...
[Functions]
  [bc_func]
    type = ParsedFunction
    expression = 'LHR / ( 2 * pi * fuel_radius * water_heat_trans_coeff) + deltaT*y/core_height + T_in'
    symbol_names = 'LHR fuel_radius water_heat_trans_coeff deltaT core_height T_in'
    symbol_values = '${LHR} ${fuel_radius} ${water_heat_trans_coeff} ${deltaT} ${core_height} ${T_in}'
  []
[]
