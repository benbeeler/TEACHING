#
# MOOSE PROJECT PART 2 input Steady State with temperature-dependent K
#
...
[Kernels]
  ...
  [heat_source]
    type = HeatSource
    variable = temperature
    function = '(350 * cos(1.2*((y/50)-1)))/(pi*(0.5^2))'  # Axial LHR variation
	block = 1
  []
[]
...
[BCs]
  ...
  [outer_temperature]
    ...
    variable = temperature
    boundary = 'right'
    function = '(500 + ((1/1.2)*((50*350)/(250*4.2))*(sin (1.2)+ (sin (1.2*((y/50)-1))))))+((350 * cos(1.2*((y/50)-1)))/(2*pi*0.5*2.65)) '  # Axial Tcool variation
  []
[]
...
[Outputs]
  ...
  [csv]
    type = CSV
    file_base = mp2_steady_k_t_test_out
    execute_on = final
  []
[]
